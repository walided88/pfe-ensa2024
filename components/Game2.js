...
import GameView from '../view/Game2View';
import { incrementScoreAddition } from '../services/Database';
import CollisionHandler from '../systems/CollisionHandler';

const { width, height } = Dimensions.get('window');

// Composant Game utilisant forwardRef pour permettre l'accès aux fonctions internes depuis l'extérieur
const Game = forwardRef((props, ref) => {
  const dispatch = useDispatch();
  const position = useRef(new Animated.ValueXY({ x: 300, y: 200 })).current; // Position animée du personnage
  ...
  const [currentImage, setCurrentImage] = useState(require('../assets/images/fChefront.webp')); // Image actuelle du personnage
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      Matter.Engine.update(engine.current, 1000 / 40);
  ...
  const moveBoxToPosition = useCallback((x, y) => {
    ...
    if (x < currentPos.x) {
      setCurrentImage(require('../assets/images/fChefL.webp'));
      setLastDirection('left');
    } else if (x > currentPos.x) {
      setCurrentImage(require('../assets/images/fChefR.webp'));
  ...
  useEffect(() => {
    if (!isAnimating) {
      switch (lastDirection) {
        case 'left':
          setCurrentImage(require('../assets/images/fChefront.webp'));
          console.log("add yyyyyyyyyyy", add);
          break;
        case 'right':
          setCurrentImage(require('../assets/images/fChefront.webp'));
          break;
